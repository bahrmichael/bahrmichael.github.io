...
Serverless Cloud is a new initiative by Serverless Inc. and [entered public preview on October 13th, 2021](https://twitter.com/goserverless/status/1448313005331005443).
...
Earlier in 2021 I joined the beta program where tried out Serverless Cloud in its early stages (v0.15).
...
When you code runs on that instance, it calls a database to get data. For this article we will call the services that your code uses "building blocks".
...
They provide these via the built-in helpers `api`, `schedule`, `data`, and `params`. The data events are part of the `data` helper.
...
If you already have experience with express.js, the `api` helper will feel familiar to you. We specify a route, handle a request object, and send back a response.
...
[Check out the API documentation.](https://www.serverless.com/cloud/docs/apps/api)
...
Please note that these functions have a default timeout of 60 seconds, and at most 300 seconds (5 minutes).

I didn't use the scheduler for my project, but noticed that there's a
[helper for triggering scheduled functions in integration tests](https://www.serverless.com/cloud/docs/workflows/testing#schedule-helpers).
...
Furthermore, you can use so labels to enable access patterns through fields that are not part of the primary index.
...
It looks like the DynamoDB single table design influence the `data` helper.
...
But there don't seem to be compile time validations yet. I'd like to suggest some form of linter, or a `namespace` method like below.
An alternative might be a data modelling library like [dynamodb-toolbox](https://github.com/jeremydaly/dynamodb-toolbox).
...
`data` also comes with a time to live attribute, after which the records are not available through get requests anymore.
...
```typescript
const requestCount = (await data.get(`apirate:push:*`, {limit: 90}))?.items?.length ?? 0;
```
...
When our application creates, updates, or deletes and item, the runtime calls our event handler.
...
It gathers more information about a route, so that we can show more advanced data on further requests.
...
Define the secret with a placeholder, and then override it in production.
...
With the `.serverlessignore` file we can exclude it from the deployment.
...
Here's an example what I use for Next.js, where the frontend code lives in the folder `./frontend`:

`next build && next export && rm -Rf ../static && cp -Rf out ../static`

The build step above adds about 15 seconds, and may increase as my frontend grows. But this is completely under my control.
...
Integration tests and beyond should run on the cloud, as I don't 100% trust emulated service dependencies.
...
I have not tested if it also deploys updates from files that the `index.ts` file imports.
...
With the paid plans there seems to be no limit on how many instances you can have.
...
This should be very helpful when debugging emerging problems. I feel like this increases the necessity for protected stages.
...
but I expect them to "just go away" as Serverless Cloud approaches general availability. This applies to both personal instance and stages.
...
When I wrote this article, Emrah from the Serverless Cloud helped me root out some errors. Please let me know if you find more!
